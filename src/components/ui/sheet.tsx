...
const SheetContent = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Content>, SheetContentProps>(({ side = 'right', className, children, title, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
      {/* Insert an accessible SheetTitle if provided, else a visually hidden fallback */}
      {title ? <SheetTitle>{title}</SheetTitle> : <SheetTitle className="sr-only">Sheet</SheetTitle>}
      {children}
